...
class SalesOrderSavedPage extends StatefulWidget {
  const SalesOrderSavedPage({super.key});

  @override
  State<SalesOrderSavedPage> createState() => _SalesOrderSavedPageState();
}

class _SalesOrderSavedPageState extends State<SalesOrderSavedPage> {
  String userId = '';
  String userName = '';
  ...
  final Map itemIdMap ={};
  final Map itemUnitPriceMap = {};
  final Map amountMap = {};
  final Map descriptionItemIdMap = {};
    ...
    setState(() {
      ...
      if (cartItems != null) {
        ...
        for (var value in cartItemsJsonForLoops) {
          itemIdMap['amount_${value['item_id']}'] = value['item_id'];
          amountMap['amount_${value['item_id']}'] = value['quantity'];
          itemUnitPriceMap['amount_${value['item_id']}'] = value['item_unit_price'];
        }
      }
      ...
      if (descriptionItems != null) {
        ...
        for (var value in descriptionItemsJsonForLoops) {
          descriptionItemIdMap['description_${value['item_id']}'] = value['item_id'];
          descriptionMap['description_${value['item_id']}'] = value['description'];
        }
      }
      salesInvoiceId = prefs.getString('salesInvoiceId')!;
      subtotal = prefs.getString('subtotal')!;
      subtotalItem = prefs.getString('subtotalItem')!;
      total = prefs.getString('totalAmount')!;
        ...
        return orientation == Orientation.landscape ? LayoutBuilder(
          ...
          onWillPop: () async {
            ...
            /*Navigator.of(context).pushAndRemoveUntil(
              CupertinoPageRoute(
                builder: (context) {
                  return const SalesDetailSavedPage();
                },
              ), (_) => false,
            );
            return Future.value(false);*/
                ...
                Container(
                  ...
                  child: Center(
                    child: TextFormField(
                      ...
                      initialValue: amountMap['amount_${itemJson[index]['item_id']}'] != null ? amountMap['amount_${itemJson[index]['item_id']}'].toString() : "0",
                      key: Key(amountMap['amount_${itemJson[index]['item_id']}'] != null ? amountMap['amount_${itemJson[index]['item_id']}'].toString() : "0",
                      ),
                      validator: (value) {
  ...
  addItemManual(var itemId, var itemName, var itemUnitPrice, var inputAmount) async {
    final prefs = await SharedPreferences.getInstance();
    cartItems = prefs.getString('cartitems');
    bool cek = false;
    var quantity = 0;
  ...
  addDescription(var itemId, var value) async {
    ...
    setState(() {
      descriptionItemIdMap['description_$itemId'] = itemId;
                    ...
                    children: [
                      Expanded(
                          child: Container(
                            ...
                            child: TextFormField(
                              maxLines: 4,
                              initialValue: descriptionMap['description_$itemId'] != null ? descriptionMap['description_$itemId'].toString() : "",
                              key: Key(descriptionMap['description_$itemId'] != null ? descriptionMap['description_$itemId'].toString() : ""),
                              onChanged: (value) {
  ...
  Future<void> updateUnsavedOrder(context) async {
    final prefs = await SharedPreferences.getInstance();
    showLoaderDialog(context);
    token = prefs.getString('token')!;
    try {
      ...
      if (cartItems != null) {
        cartItemsJsonForLoops = json.decode(cartItems!);
        for (var value in cartItemsJsonForLoops) {
          formData['items[${value['item_id']}][item_id]'] = value['item_id'];
          formData['items[${value['item_id']}][quantity]'] = value['quantity'];
          formData['items[${value['item_id']}][item_unit_price]'] = value['item_unit_price'];
        }
      }
      descriptionItems = prefs.getString('descriptionitems');
      if (descriptionItems != null) {
        descriptionItemsJsonForLoops = json.decode(descriptionItems!);
        for (var value in descriptionItemsJsonForLoops) {
          formData['descriptions[${value['item_id']}][item_id]'] = value['item_id'];
          formData['descriptions[${value['item_id']}][description]'] = value['description'];
        }
      }

      /*Static Value*/
      formData['user_id'] = userId;
      formData['sales_invoice_id'] = json.decode(salesInvoiceId);
      formData['subtotal_amount'] = subtotal;
      formData['subtotal_item'] = subtotalItem;
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        // berhasil
        hideLoaderDialog(context);
        /*jsonPrint(json.decode(cartItems!));*/
        /*setState(() {
          savedItem = json.encode(response.data['data']);
          jsonPrint('Update data');
          jsonPrint(json.decode(savedItem));
        });*/
        await prefs.setString('cartitems', cartItems!);
        await prefs.setString('descriptionitems', descriptionItems!);
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (context) => const MainBottomNavigation()),
        );
        CustomSnackbar.show(context, 'Pesanan Berhasil Diperbarui');
      }
    } on DioException catch (e) {
      hideLoaderDialog(context);
      if (e.response?.statusCode == 400 || e.response?.statusCode == 401) {
