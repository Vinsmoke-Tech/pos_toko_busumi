...
class _MainPageState extends State<MainPage> {
                      ...
                      Expanded(
                        child: Row(
                          children: [
                            // Left Column
                            SizedBox(
                                width: columnWidth, // Adjust as needed
                                child: Column(
                                  children: [
                                    Container(
                                    ...
                                    Flexible(
                                      child: unpaidsalesinvoiceJson.isNotEmpty ? ListView.builder(
                                        padding: const EdgeInsets.only(bottom: 100),
                                        physics: const ClampingScrollPhysics(),
                                        shrinkWrap: true,
                                        itemCount: unpaidsalesinvoiceJson.length,
                                        itemBuilder: (context, int index) {
                                          return makeCardUnpaid(context, index);
                                ...
                                children: [
                                  Card(
                                    ...
                                    child: ExpansionTile(
                                      ...
                                      children: [
                                        unpaidsalesinvoiceJson.isNotEmpty ? ListView.builder(
                                          physics: const ClampingScrollPhysics(),
                                          shrinkWrap: true,
                                          itemCount: unpaidsalesinvoiceJson.length,
                                          itemBuilder: (context, int index) {
        ...
        leading: Container(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: const Color(0xffff76be).withOpacity(0.7),
                ...
                showDialog(
                    ...
                    builder: (context) {
                      return AlertDialog(
                        ...
                        content: const Text(
                          'Apakah anda yakin ingin menghapus pesanan tersimpan?',
                          style: TextStyle(
                              color: Colors.black,
                              fontSize: 14
                          ),
                        ...
                        actions: [
                          ...
                          ElevatedButton(
                            onPressed: () {
                              deleteSalesSave(context, unpaidsalesinvoiceJson[index]['sales_invoice_id']);
                            },
                            child: const Text(
                              'Hapus',
                              style: TextStyle(
  ...
  Future<void> fetchUnPaidSalesToday(context) async {
    // Remove data for the 'counter' key.
    final prefs = await SharedPreferences.getInstance();
    // showLoaderDialog(context);
    token = prefs.getString('token')!;
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        setState(() {
          ...
          jsonPrint(unpaidsalesinvoiceJson);
        });
      }
    } on DioException catch (e) {
      objectPrint(e);
